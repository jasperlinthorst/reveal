...
def main():
    ...
    parser_inversions = subparsers.add_parser('inversions', prog="reveal inversions", description="Representating inversions in a DAG can be cumbersome, with this we can detect and \'undo\' the inversions with respect to a reference sequence.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    parser_finish.add_argument("--maxgapsize", dest="maxgapsize", type=int, default=20000, help="Maxgapsize between MUMs before breaking chain.")
    parser_finish.add_argument("--minchainlength", dest="minchainlength", type=int, default=100,help="Minimal length of a chain on the reference before its reported.")
    parser_finish.add_argument("--minchainsum", dest="minchainsum", type=int, default=65,help="Minimal sum of the length of the MUMs in a chain before its reported.")
    ...
    parser_inversions.add_argument('reference', help='Multi-fasta reference sequence.')
    parser_inversions.add_argument('contigs', help='Multi-fasta (finished) assembly in which contained inversions are to be detected and removed.')
    parser_inversions.add_argument("-o", "--output", dest="output", help="Prefix of fasta file for the \'finished\' genome.")
    parser_inversions.add_argument("-m", dest="minlength", type=int, default=20, help="Min length of maximal exact matches for considering.")
    parser_inversions.add_argument("-i", dest="interactive", action="store_true", default=False, help="Output interactive plot.")
    parser_inversions.add_argument("--plot", dest="plot", action="store_true", default=False, help="Output mumplots for the \'finished\' chromosomes (depends on matplotlib).")
    parser_inversions.add_argument("--plotall", dest="plotall", action="store_true", default=False, help="Plot all matches, instead of only the chained matches.")
    parser_inversions.add_argument("--split", dest="split", action="store_true", default=False, help="Split the \'finished\' genome by chromosome.")
    parser_inversions.add_argument("--gapsize", dest="gapsize", type=int, default=1, help="Use this number of N's to flank the inverted sequence.")
    parser_inversions.set_defaults(func=finish.inversions)
