...
def main():
    ...
    parser_aln.add_argument("-n", dest="minn", type=int, default=2, help="Only align graph on exact matches that occur in at least this many samples (if not set, equal to total number of genomes in the (sub)index).")
    ...
    parser_finish.add_argument("-m", dest="minlength", type=int, default=0, help="Min length of maximal exact matches for considering (if not set, use the set of largest MUMs for which the genome wide coverage is below 1).")
    ...
    parser_finish.add_argument("--mineventsize", dest="mineventsize", type=int, default=None, help="Minimal size (in basepairs) of a rearrangement event that has to be transformed (set larger than typical transposon event, if not set, derive from MUM coverage).")
